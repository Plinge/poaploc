...
import numpy as np
import soundfile
from config import GEO, AUDIOPATH
import scipy.signal
...
for r,a,t60 in itertools.product(range(2),range(0,360,5),[0.3,0.6]):
    ...
    if os.path.exists(rirfilename):
        ...
        rirs = RG.rir_generator(SPEEDOFSOUND, FS, MIC_POS + offset, s, ROOM, beta=t60, nsample=NUMSAMPLES,mtype='omnidirectional', hp_filter = 1)
        print rirs.shape 
        np.save(rirfilename,rirs)
        
    SIGNAL = np.random.random((FS*1.2,))
    print 'convolving'
    out=[]
    for i in xrange(8):
