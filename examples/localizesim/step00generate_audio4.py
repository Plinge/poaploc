...
ROOM5        = np.array([3.2,3.8,2.6])     # Room dimensions [x y z] (m)
MIC_CENTER   = np.array([1.8,2.15,1.0]) 
MIC_POS      = MIC_CENTER + GEO # Receiver position [x y z] (m)

make_sure_path_exists(RIRPATH)
make_sure_path_exists(AUDIOPATH)

for r,t60,a in itertools.product([24,],[0.33,0.48,0.63,0.78,0.93],range(360,0,-1)):
